...
export class ConfigService {
  ...
  constructor(firestore: Firestore, authService: AuthService) {
    ...
    this.observableConfig = new Observable((sub: Subscriber<Config>) => {
      onSnapshot(this.ref, (doc) => {
        if (doc.exists()) {
          const config: Config = this.nestArrays(doc.data());
          console.log(config);
          sub.next(config);
          return;
        }
  ...
  setInitialConfig() {
    setDoc(this.ref, this.removeNestedArrays(this.initalConfig));
  }

  private subConfig(sub: Subscriber<Config>) {
    return this.observableConfig.subscribe(sub);
  }
  ...
  removeNestedArrays(obj: any) {
    ...
    if (Array.isArray(obj)) {
      obj.forEach((arrElement: any, idx: number) => {
        newObj[idx.toString(10)] = this.removeNestedArrays(arrElement);
      });
      return newObj;
    }
    Object.entries(obj).forEach(([k, v]) => {
      newObj[k] = this.removeNestedArrays(v);
    });
    return newObj;
  }
  ...
  nestArrays(obj: any) {
    ...
    if (isArray()) {
      let arr: any[] = [];
      Object.entries(obj).forEach(([k, v]) => {
        arr[parseInt(k)] = this.nestArrays(v);
      });
      return arr;
    }
    let newObj: any = {};
    Object.entries(obj).forEach(([k, v]) => {
      newObj[k] = this.nestArrays(v);
    });
    return newObj;
  }
}
